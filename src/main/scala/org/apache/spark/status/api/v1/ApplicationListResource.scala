...
private[v1] class ApplicationListResource extends ApiRequestContext with Logging {
  ...
  : Iterator[ApplicationInfo] = {
	...
	// jsingh 8/26/2023 - changes for tenant level ui filtering by passing tenant id set from authentication to application listing function
	val tenantId=httpRequest.getAttribute("TenantId").asInstanceOf[String]
	logDebug("*** TenantId="+tenantId)
    uiRoot.getApplicationInfoList(tenantId).filter { app =>
